...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { eventName?: string; path?: string } | null;
  ...
  if (eventName === "session_start") await recordAuthEvent(supabase, "session_start", { path });
