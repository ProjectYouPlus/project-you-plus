...
export async function POST(request: Request) {
  ...
  if (!user)
    return NextResponse.json(
      { error: "Sign in again to use Nutrition AI." },
      { status: 401 },
    );
  ...
  if (!isOpenAIConfigured()) {
    return NextResponse.json(
      {
        error: "Nutrition AI cloud vision is not configured yet.",
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Choose a JPEG, PNG, or WEBP meal photo." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          "Nutrition AI could not analyze that photo. You can retry or enter the meal manually.",
      },
      { status: 500 },
    );
  }
...
function sanitize(result: NutritionAIResult): NutritionAIResult {
  if (
    ...
    throw new Error("Invalid nutrition analysis");
  const validated = validateFoods(result.foods);
  return {
    mealName: String(result.mealName || "Meal").slice(0, 80),
    confidence: Math.max(0, Math.min(1, Number(result.confidence) || 0)),
    foods: validated.slice(0, 12).map((food) => ({
    ...
    note: String(result.note || "Review the estimate before saving.").slice(
      0,
      240,
    ),
  };
