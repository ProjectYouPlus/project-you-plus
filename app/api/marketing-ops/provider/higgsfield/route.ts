...
export async function POST(request: Request) {
  const { allowed, user } = await requireMarketingOwner();
  if (!allowed || !user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const admin = createAdminClient();
  let secret;
  try {
    secret = await getHiggsfieldSecret(user.id);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Higgsfield is not connected" }, { status: 409 });
  }
  ...
  for (const raw of jobs || []) {
    ...
    try {
      results.push(job.status === "awaiting_provider"
        ? await submitJob(admin, secret, job)
        : await pollJob(admin, secret, job));
    } catch (reason) {
      const message = reason instanceof Error ? reason.message : "Higgsfield provider step failed";
      await admin.from("marketing_generation_jobs").update({
...
async function submitJob(admin: ReturnType<typeof createAdminClient>, secret: Awaited<ReturnType<typeof getHiggsfieldSecret>>, job: Job) {
  ...
  const { error: reserveError } = await admin.rpc("reserve_marketing_generation_estimate", {
    p_job_id: job.id,
    p_estimate: estimate.credits,
  });
  ...
  await admin.from("marketing_content_items").update({
    ...
    sub_status: "provider_processing",
    next_action: "Higgsfield is generating the creative asset",
    updated_at: new Date().toISOString(),
  }).eq("id", job.content_item_id);
  await logActivity(admin, job, "provider_submitted", `Higgsfield accepted the ${job.generation_type} generation.`, {
    job_id: job.id,
    provider_job_id: submitted.requestId,
    model_path: modelPath,
...
async function pollJob(admin: ReturnType<typeof createAdminClient>, secret: Awaited<ReturnType<typeof getHiggsfieldSecret>>, job: Job) {
  ...
  if (["completed", "succeeded", "success"].includes(normalized)) {
    ...
    const { error: finishError } = await admin.from("marketing_generation_jobs").update({
      status: "succeeded",
      actual_credits: actualCredits,
      asset_url: savedAssetUrl,
      error_message: null,
      metadata: { ...(job.metadata || {}), provider_status: result.status, provider_asset_url: result.assetUrl, completed_at: new Date().toISOString() },
      updated_at: new Date().toISOString(),
    }).eq("id", job.id);
    ...
    const contentUpdate: Record<string, unknown> = {
      ...
      sub_status: "quality_review",
      next_action: "Run Muse quality review",
      blocked_reason: null,
      error_message: null,
      updated_at: new Date().toISOString(),
    };
    ...
    await logActivity(admin, job, "provider_completed", `Higgsfield completed the ${job.generation_type}; the asset is saved permanently in Project You+.`, {
      job_id: job.id,
  ...
  if (["failed", "nsfw", "canceled", "cancelled"].includes(normalized)) {
    ...
    await admin.from("marketing_generation_jobs").update({
      status: terminal,
      error_message: message,
      metadata: { ...(job.metadata || {}), provider_status: result.status, completed_at: new Date().toISOString() },
      updated_at: new Date().toISOString(),
    }).eq("id", job.id);
    await admin.from("marketing_content_items").update({
      sub_status: "generation_failed",
      blocked_reason: message,
      error_message: message,
      next_action: "Review the creative brief before retrying generation",
      updated_at: new Date().toISOString(),
    }).eq("id", job.content_item_id);
  ...
  await admin.from("marketing_generation_jobs").update({
    status: nextStatus,
    metadata: { ...(job.metadata || {}), provider_status: result.status, last_polled_at: new Date().toISOString() },
    updated_at: new Date().toISOString(),
  }).eq("id", job.id);
  return { jobId: job.id, status: nextStatus, providerStatus: result.status };
}

async function persistProviderAsset(admin: ReturnType<typeof createAdminClient>, job: Job, remoteUrl: string) {
...
async function logActivity(admin: ReturnType<typeof createAdminClient>, job: Job, eventType: string, message: string, metadata: Record<string, unknown>) {
