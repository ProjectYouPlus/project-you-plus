...
type ActivePlan = { id: string; title: string; goal: string; days_per_week: number; schedule: PlanSession[] | null };
...
export default async function DashboardPage() {
  ...
  const [profile, goals, tasks, habits, live] = await Promise.all([
    getProfile(), getGoals(), getTasks(), getHabits(), getLiveDashboardData(supabase, rangeStart, rangeEnd),
  ]);
  ...
  const nutritionByDay=nutritionMap(live.nutritionLogs);
  const todaySession=sessionForDate(live.activePlan,now);
  ...
  const projectedScore=todayCommitments.length?Math.max(currentScore,weightedScore(pillars.map((pillar)=>pillar.key==="Execution"?{...pillar,value:100}:pillar))):currentScore;
  ...
  const remainingCount=todayCommitments.filter((item)=>!item.done).length;
...
async function getLiveDashboardData(supabase:Awaited<ReturnType<typeof createClient>>,start:Date,end:Date){
  const startIso=start.toISOString(),endIso=end.toISOString(),startDate=localDate(start),endDate=localDate(end);
  ...
  const [habitLogs,workouts,activePlanRes,planLogs,supplements,supplementLogs,metrics,accounts,transactions,budgets,nutritionLogs,events,workSchedules,integrations,connections,challenges,alerts]=await Promise.all([
    ...
    supabase.from("workout_plan_logs").select("plan_id,session_key,completed_on,duration_minutes").gte("completed_on",startDate).lte("completed_on",endDate),
...
function buildDays(args:{start:Date;end:Date;today:string;tasks:Task[];habits:Habit[];activePlan:ActivePlan|null;supplements:Supplement[];habitLogMap:Map<string,Set<string>>;planLogMap:Set<string>;supplementLogMap:Map<string,Set<string>>;nutritionByDay:Map<string,{calories:number;protein:number;carbs:number;fat:number;meals:number}>;events:Array<{id:string;title:string;start_at:string;end_at:string;location:string|null;source:string|null}>;workSchedules:WorkSchedule[];goalById:Map<string,Goal>;topTasks:Task[]}){
  ...
  while(cursor<=last){
    ...
    const taskPool=isToday?uniqueTasks([...args.tasks.filter((task)=>task.dueAt&&localDate(new Date(task.dueAt))===key),...args.topTasks]):args.tasks.filter((task)=>task.dueAt&&localDate(new Date(task.dueAt))===key||task.completedAt&&localDate(new Date(task.completedAt))===key);
    const tasks:DashboardTask[]=uniqueTasks(taskPool).map((task)=>({id:task.id,title:task.title,tier:task.tier,done:Boolean(task.completedAt&&localDate(new Date(task.completedAt))<=key),goalTitle:task.goalId?args.goalById.get(task.goalId)?.title:null,dueAt:task.dueAt,recommended:isToday&&args.topTasks.some((top)=>top.id===task.id)}));
    ...
    const session=sessionForDate(args.activePlan,cursor);const workout=session&&args.activePlan?{planId:args.activePlan.id,sessionKey:session.key,title:session.title,focus:session.focus??null,duration:session.duration,done:args.planLogMap.has(`${key}:${args.activePlan.id}:${session.key}`)}:null;
    const supplements=args.supplements.filter((item)=>isSupplementDue(item,cursor,Boolean(session))).map((item)=>({id:item.id,name:item.name,dosage:item.dosage,timing:item.timing,done:hasValue(args.supplementLogMap,key,item.id)}));
    const events=args.events.filter((event)=>localDate(new Date(event.start_at))===key).map((event)=>({id:event.id,title:event.title,startAt:event.start_at,endAt:event.end_at,source:event.source,location:event.location}));
...
function sessionForDate(plan:ActivePlan|null,date:Date){return plan?.schedule?.find((session)=>session.dayIndex===date.getDay())??null}
function isSupplementDue(item:Supplement,date:Date,hasTraining:boolean){if(item.frequency==="daily")return true;if(item.frequency==="weekdays")return date.getDay()>=1&&date.getDay()<=5;if(item.frequency==="training_days")return hasTraining;if(item.frequency==="weekly")return date.getDay()===1;return false}
...
function consistencyScore(live:Awaited<ReturnType<typeof getLiveDashboardData>>,habits:Habit[],start:Date,end:Date){const days=Math.max(1,Math.min(7,Math.floor((end.getTime()-start.getTime())/86400000)+1));const startDate=localDate(start);const habitExpected=habits.filter((habit)=>habit.targetFrequency==="daily").length*days;const habitActual=live.habitLogs.filter((row)=>row.logged_at>=startDate).length;const workoutExpected=live.activePlan?.days_per_week??0;const workoutActual=live.planLogs.filter((row)=>row.completed_on>=startDate).length;const supplementExpected=live.supplements.filter((item)=>item.frequency==="daily").length*days;const supplementActual=live.supplementLogs.filter((row)=>row.logged_on>=startDate).length;const expected=habitExpected+workoutExpected+supplementExpected;return expected?Math.min(100,pct(habitActual+Math.min(workoutActual,workoutExpected)+supplementActual,expected)):null}
function bodyScore(live:Awaited<ReturnType<typeof getLiveDashboardData>>,start:Date,end:Date){const latest=new Map<string,number>();for(const metric of live.metrics)if(!latest.has(metric.metric_type))latest.set(metric.metric_type,Number(metric.value));const parts:number[]=[];if(latest.has("steps"))parts.push(Math.min(100,pct(latest.get("steps")!,10000)));if(latest.has("sleep_minutes"))parts.push(Math.min(100,pct(latest.get("sleep_minutes")!,480)));if(latest.has("water_cups"))parts.push(Math.min(100,pct(latest.get("water_cups")!,10)));if(live.activePlan){const startDate=localDate(start),endDate=localDate(end);const completed=live.planLogs.filter((row)=>row.completed_on>=startDate&&row.completed_on<=endDate).length;parts.push(Math.min(100,pct(completed,Math.max(1,live.activePlan.days_per_week))))}return parts.length?Math.round(parts.reduce((sum,value)=>sum+value,0)/parts.length):null}
...
function nutritionMap(rows:Array<{calories:number;protein_g:number|string;carbs_g:number|string;fat_g:number|string;logged_at:string}>){const map=new Map<string,{calories:number;protein:number;carbs:number;fat:number;meals:number}>();for(const row of rows){const day=localDate(new Date(row.logged_at));const current=map.get(day)??{calories:0,protein:0,carbs:0,fat:0,meals:0};current.calories+=Number(row.calories||0);current.protein+=Number(row.protein_g||0);current.carbs+=Number(row.carbs_g||0);current.fat+=Number(row.fat_g||0);current.meals+=1;map.set(day,current)}return map}
