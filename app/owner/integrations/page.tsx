...
export default async function OwnerIntegrationsPage({searchParams}:PageProps){
  ...
  const hfVideoNotice=typeof searchParams?.higgsfield_video==="string"?searchParams.higgsfield_video:null;
  const videoReady=Boolean(higgsfield?.metadata?.video_generation_ready);
  return <OwnerSectionShell active="integrations" title="Integrations" subtitle="Connected services recorded by Project You+. Secrets are never displayed here." displayName={displayName} approvalCount={approvalCount}>
    ...
    {hfVideoNotice?<div className="mt-4 rounded-lg border border-amber-400/20 bg-amber-500/[.06] px-4 py-3 text-[10px] text-amber-200">Higgsfield image generation is ready. Add the exact text-to-video endpoint available to your Cloud account before automatic Reel generation.</div>:null}
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-2">
      ...
      <OwnerPanel title="Higgsfield Creative Engine" subtitle="Server-side image and video generation for Frame and Muse. Credentials stay in the private integration secret store.">
        {higgsfield?.status==="connected"?<div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/[.045] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-[12px] font-semibold text-emerald-200">Higgsfield Cloud connected</div><div className="mt-1 text-[9px] leading-4 text-[#8b96aa]">Image: {String(higgsfield.metadata?.image_model_path||"configured")}<br/>Video: {videoReady?String(higgsfield.metadata?.video_model_path||"configured"):"endpoint still required"}</div></div><div className="flex gap-2"><Link href="/owner/marketing" className="rounded-lg border border-[#5f47b7] bg-[#211644] px-4 py-2 text-[9px] text-[#d7cbff]">Open production</Link><form action="/api/integrations/higgsfield/connect" method="post"><input type="hidden" name="action" value="disconnect"/><button className="rounded-lg border border-red-400/20 bg-red-500/[.06] px-4 py-2 text-[9px] text-red-200">Disconnect</button></form></div></div></div>:<div className="mt-3 rounded-xl border border-[#26344b] bg-[#0a1320] p-4"><div className="text-[10px] leading-5 text-[#9aa6ba]">For autonomous generation, Project You+ needs Higgsfield Cloud server credentials. The ChatGPT Higgsfield connection cannot be reused by Vercel. Image generation uses Higgsfield's documented Soul endpoint; paste your account's exact text-to-video endpoint below if you want automatic Reel generation too.</div><form action="/api/integrations/higgsfield/connect" method="post" className="mt-4 grid gap-3"><label className="text-[9px] text-[#8793a8]">API key ID<input name="apiKeyId" required autoComplete="off" className="mt-1 w-full rounded-lg border border-[#2b3950] bg-[#070d16] px-3 py-2.5 text-[10px] text-white outline-none focus:border-[#7153d4]"/></label><label className="text-[9px] text-[#8793a8]">API key secret<input name="apiKeySecret" type="password" required autoComplete="new-password" className="mt-1 w-full rounded-lg border border-[#2b3950] bg-[#070d16] px-3 py-2.5 text-[10px] text-white outline-none focus:border-[#7153d4]"/></label><label className="text-[9px] text-[#8793a8]">Text-to-video endpoint <span className="text-[#59677d]">(optional)</span><input name="videoModelPath" autoComplete="off" placeholder="provider/model/path from Higgsfield Cloud" className="mt-1 w-full rounded-lg border border-[#2b3950] bg-[#070d16] px-3 py-2.5 text-[10px] text-white outline-none focus:border-[#7153d4]"/></label><div className="flex flex-wrap items-center gap-2"><button className="rounded-lg border border-[#8b63ff] bg-[linear-gradient(180deg,#4a2c9c,#2b195f)] px-4 py-2.5 text-[9px] font-semibold text-white">Connect & test</button><a href="https://cloud.higgsfield.ai" target="_blank" rel="noreferrer" className="rounded-lg border border-[#30405a] bg-[#0b1422] px-4 py-2.5 text-[9px] text-[#aab5c7]">Create API credentials ↗</a></div></form></div>}
      </OwnerPanel>
    </div>
