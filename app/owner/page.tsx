...
type Login = { id:string; user_id:string; occurred_at:string; city:string|null; region:string|null; country:string|null; latitude:number|string|null; longitude:number|string|null; device_family:string|null; browser:string|null; os:string|null };
type Module = { module_key:string; label:string; enabled:boolean; rollout_percent:number; locked:boolean };
type GrowthPoint = { key:number; label:string; total:number; newUsers:number; signIns:number };
...
export default async function OwnerPage(){
  ...
  const growth=buildGrowth(users,logins,now);
  const locations=rank(logins.map(locationLabel).filter(v=>v!=="Unknown")).slice(0,6);
  const mapPoints=buildMapPoints(logins);
  ...
  return <OwnerSectionShell
    active="overview"
    title="Owner Command Center"
    subtitle="Real users. Real data. A better tomorrow."
    displayName={displayName}
    approvalCount={approvalCount}
    actions={<div className="hidden gap-3 md:flex"><button type="button" className="rounded-lg border border-[#35415a] bg-[#0d1422] px-4 py-2 text-[11px]">Last 30 days⌄</button><Link href="/owner/analytics" className="rounded-lg border border-[#6942d8] bg-[#160f2d] px-4 py-2 text-[11px] text-[#c8b6ff]">View Analytics</Link></div>}
  >
...
function buildGrowth(users:User[],logins:Login[],now:number):GrowthPoint[]{const days=30;const today=new Date(now);today.setHours(0,0,0,0);const first=today.getTime()-(days-1)*DAY;return Array.from({length:days},(_,key)=>{const dayStart=first+key*DAY;const dayEnd=dayStart+DAY-1;return{key,label:new Date(dayEnd).toLocaleDateString("en-US",{month:"short",day:"numeric"}),total:users.filter(u=>new Date(u.created_at).getTime()<=dayEnd).length,newUsers:users.filter(u=>{const t=new Date(u.created_at).getTime();return t>=dayStart&&t<=dayEnd}).length,signIns:logins.filter(l=>{const t=new Date(l.occurred_at).getTime();return t>=dayStart&&t<=dayEnd}).length}})}
function buildMapPoints(logins:Login[]):GeoPoint[]{const points=new Map<string,GeoPoint>();for(const login of logins){const latitude=Number(login.latitude),longitude=Number(login.longitude);if(!Number.isFinite(latitude)||!Number.isFinite(longitude)||latitude<-90||latitude>90||longitude<-180||longitude>180)continue;const label=locationLabel(login);const key=`${latitude.toFixed(3)}:${longitude.toFixed(3)}:${label}`;const current=points.get(key);if(current)current.count+=1;else points.set(key,{key,label,latitude,longitude,count:1})}return [...points.values()].sort((a,b)=>b.count-a.count).slice(0,12)}

function UserGrowthChart({rows}:{rows:GrowthPoint[]}){const width=720,height=190,pad=18,bottom=156,maxTotal=Math.max(1,...rows.map(row=>row.total)),maxSignIns=Math.max(1,...rows.map(row=>row.signIns)),x=(index:number)=>pad+index*((width-pad*2)/Math.max(1,rows.length-1)),y=(value:number)=>bottom-(value/maxTotal)*(bottom-pad),points=rows.map((row,index)=>`${x(index)},${y(row.total)}`).join(" "),area=`${pad},${bottom} ${points} ${width-pad},${bottom}`,newTotal=rows.reduce((sum,row)=>sum+row.newUsers,0),signInTotal=rows.reduce((sum,row)=>sum+row.signIns,0);return <div className="mt-3"><div className="mb-2 flex flex-wrap items-end justify-between gap-3"><div><div className="text-[24px] font-bold tracking-[-.04em]">{rows.at(-1)?.total??0}</div><div className="text-[9px] text-[#77849a]">total accounts</div></div><div className="flex gap-4 text-right text-[9px]"><div><b className="block text-[12px] text-[#cfc2ff]">+{newTotal}</b><span className="text-[#77849a]">new · 30d</span></div><div><b className="block text-[12px] text-[#8bb8ff]">{signInTotal}</b><span className="text-[#77849a]">sign-ins · 30d</span></div></div></div><div className="overflow-hidden rounded-lg border border-[#202d43] bg-[#07101d]"><svg role="img" aria-label="Actual cumulative user growth and sign-in activity for the last 30 days" viewBox={`0 0 ${width} ${height}`} className="h-[180px] w-full"><defs><linearGradient id="ownerGrowthFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#8b5cf6" stopOpacity=".35"/><stop offset="1" stopColor="#8b5cf6" stopOpacity="0"/></linearGradient></defs>{[.25,.5,.75,1].map(level=><line key={level} x1={pad} x2={width-pad} y1={bottom-level*(bottom-pad)} y2={bottom-level*(bottom-pad)} stroke="#253249" strokeWidth="1" strokeDasharray="3 7"/>)}{rows.map((row,index)=>{const barHeight=(row.signIns/maxSignIns)*42;return row.signIns?<rect key={row.key} x={x(index)-3} y={bottom-barHeight} width="6" height={barHeight} rx="3" fill="#4f8cf7" opacity=".65"><title>{row.label}: {row.signIns} sign-ins</title></rect>:null})}<polygon points={area} fill="url(#ownerGrowthFill)"/><polyline points={points} fill="none" stroke="#a78bfa" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>{rows.map((row,index)=>row.newUsers||index===rows.length-1?<circle key={row.key} cx={x(index)} cy={y(row.total)} r={row.newUsers?5:3.5} fill={row.newUsers?"#ddd6fe":"#8b5cf6"} stroke="#17102e" strokeWidth="2"><title>{row.label}: {row.total} total users, {row.newUsers} new, {row.signIns} sign-ins</title></circle>:null)}<text x={pad} y="180" fill="#69768c" fontSize="10">{rows[0]?.label}</text><text x={width/2} y="180" textAnchor="middle" fill="#69768c" fontSize="10">{rows[Math.floor(rows.length/2)]?.label}</text><text x={width-pad} y="180" textAnchor="end" fill="#69768c" fontSize="10">Today</text></svg></div><div className="mt-2 flex gap-4 text-[9px] text-[#8995a8]"><span><b className="text-[#a78bfa]">━</b> Total users</span><span><b className="text-[#4f8cf7]">▮</b> Real sign-ins</span>{newTotal===0?<span className="ml-auto text-[#657187]">No new accounts in this window</span>:null}</div></div>}

function SignInMap({points,totalSignIns}:{points:GeoPoint[];totalSignIns:number}){const width=720,height=330,project=(latitude:number,longitude:number)=>({x:(longitude+180)/360*width,y:(90-latitude)/180*height});return <div className="relative min-h-[220px] overflow-hidden rounded-xl border border-[#202d43] bg-[radial-gradient(circle_at_26%_42%,rgba(89,72,190,.16),transparent_34%),linear-gradient(145deg,#101a2a,#07101b)]"><div className="absolute left-3 top-3 z-10 rounded-md border border-[#34425b] bg-[#070d17]/85 px-2.5 py-1.5 text-[8px] text-[#9da9bd]">{totalSignIns} recorded sign-ins · {points.length} mapped locations</div><svg role="img" aria-label="World map of actual recorded sign-in locations" viewBox={`0 0 ${width} ${height}`} className="absolute inset-0 h-full w-full"><g fill="none" stroke="#223149" strokeWidth="1" opacity=".55">{[-120,-60,0,60,120].map(lon=><line key={`lon${lon}`} x1={(lon+180)/360*width} x2={(lon+180)/360*width} y1="0" y2={height}/>)}{[-45,0,45].map(lat=><line key={`lat${lat}`} x1="0" x2={width} y1={(90-lat)/180*height} y2={(90-lat)/180*height}/>)}</g><g fill="#172438" stroke="#35445d" strokeWidth="1.2" opacity=".9"><path d="M28 55 L75 31 136 40 184 73 218 91 239 128 214 151 177 140 154 116 126 110 94 88 55 84 Z"/><path d="M208 155 L247 165 269 208 252 274 223 309 205 248 191 199 Z"/><path d="M314 58 L365 39 411 51 425 80 399 101 351 92 319 78 Z"/><path d="M343 104 L392 111 420 153 407 220 374 274 347 234 331 170 Z"/><path d="M420 70 L495 43 594 57 668 91 650 130 578 142 535 118 487 131 451 106 Z"/><path d="M581 220 L633 213 671 248 642 283 594 276 567 247 Z"/><path d="M258 32 L286 18 309 35 297 66 268 62 Z"/></g>{points.map((point,index)=>{const p=project(point.latitude,point.longitude),radius=Math.min(10,4+Math.sqrt(point.count)*1.8);return <g key={point.key}><circle cx={p.x} cy={p.y} r={radius+7} fill="#8b5cf6" opacity=".12"/><circle cx={p.x} cy={p.y} r={radius} fill="#9f7aea" stroke="#ede9fe" strokeWidth="1.5"><title>{point.label}: {point.count} sign-ins</title></circle>{index<5?<text x={p.x+12} y={p.y-9} fill="#d8ddea" fontSize="10" paintOrder="stroke" stroke="#07101b" strokeWidth="4">{point.label} · {point.count}</text>:null}</g>})}</svg>{points.length===0?<div className="absolute inset-0 flex items-center justify-center px-8 text-center text-[9px] text-[#718098]">No sign-in coordinates have been recorded yet.</div>:null}</div>}
