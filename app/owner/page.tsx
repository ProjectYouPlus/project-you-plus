...
export default async function OwnerPage(){
  ...
  const [usersRes,metaRes,loginsRes,modulesRes,totalSigninsRes] = await Promise.all([
  ...
  return <main className="min-h-screen bg-[#050914] text-[#f5f7ff]">
    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[205px] border-r border-[#172033] bg-[#070b15]/95 lg:flex lg:flex-col">
      ...
      <nav className="mt-7 space-y-1 px-2 text-[13px] text-[#c4cbe0]">
        <Nav href="/dashboard" icon="◷" label="Today"/><Nav href="/plan" icon="□" label="Plan"/><Nav href="/coach" icon="◇" label="Coach"/><Nav href="/you" icon="♙" label="You"/>
      </nav>
      ...
      <nav className="mt-2 space-y-1 px-2 text-[12px] text-[#b7bfd3]">
        <Nav href="/owner" icon="◈" label="Overview" active/><Nav href="#users" icon="♙" label="Users"/><Nav href="#analytics" icon="▥" label="Analytics"/><Nav href="/owner/agents" icon="✣" label="AI Operations"/><Nav href="#controls" icon="⚙" label="Feature Controls"/><Nav href="#activity" icon="□" label="Content & Messaging"/><Nav href="#integrations" icon="⌘" label="Integrations"/><Nav href="#audit" icon="▤" label="Audit Log"/><Nav href="#settings" icon="⚙" label="Settings"/>
      </nav>
    ...
    <div className="relative lg:pl-[205px]">
      <header className="flex h-[65px] items-center gap-4 border-b border-[#172033] px-4 sm:px-5">
        ...
        <div className="ml-auto hidden items-center gap-3 sm:flex"><span className="rounded-lg border border-[#263249] bg-[#0b111c] px-3 py-2 text-[11px]"><span className="mr-2 text-emerald-400">●</span>Live</span><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#4b2c9a] text-[11px] font-bold">{initials(displayName)}</div><div className="text-[11px]"><div className="font-semibold">{displayName}</div><div className="text-[#8c96ab]">Owner</div></div></div>
      </header>

      <div className="px-4 py-5 sm:px-5">
        <div className="mb-4 flex items-start justify-between gap-4"><div><h1 className="text-[27px] font-bold tracking-[-.035em]">Owner Command Center</h1><p className="text-[13px] text-[#8995ae]">Real users. Real data. A better tomorrow.</p></div><div className="hidden gap-3 md:flex"><button className="rounded-lg border border-[#35415a] bg-[#0d1422] px-4 py-2 text-[11px]">Last 30 days⌄</button><button className="rounded-lg border border-[#6942d8] bg-[#160f2d] px-4 py-2 text-[11px] text-[#c8b6ff]">⇩ Export</button></div></div>

        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          <Metric icon="♙" label="Total Users" value={total} detail={`${new30} new this month`}/><Metric icon="ϟ" label="Active Users (7d)" value={active7} detail={`${pct(active7,total)}% activation rate`}/><Metric icon="♙" label="New Sign Ups (30d)" value={new30} detail={new30?`Avg. ${(new30/30).toFixed(1)}/day`:"No signups yet"}/><Metric icon="↪" label="Total Sign-ins" value={totalSignins} detail="Tracked authentication events"/><Metric icon="◎" label="Countries" value={countries} detail={locations[0]?.label?`Top: ${locations[0].label}`:"No location data yet"}/><Metric icon="▥" label="Avg. Daily Usage" value="—" detail="Session duration not tracked yet"/>
        </section>

        <section id="analytics" className="mt-3 grid gap-3 xl:grid-cols-[1.45fr_1.4fr_.95fr]">
