create or replace function private.append_onboarding_completed_event(p_session_id uuid)
returns void
language plpgsql
security definer
...
revoke all on function private.append_onboarding_completed_event(uuid) from public;
revoke all on function private.append_onboarding_completed_event(uuid) from anon;
grant execute on function private.append_onboarding_completed_event(uuid) to authenticated;

do $$
declare ddl text;
begin
  ddl:=pg_get_functiondef('public.activate_onboarding_system(uuid)'::regprocedure);
  ddl:=replace(ddl,'public.append_onboarding_completed_event(p_session_id)','private.append_onboarding_completed_event(p_session_id)');
  execute ddl;
end $$;

alter function public.activate_onboarding_system(uuid) security invoker;
...
drop function if exists public.append_onboarding_completed_event(uuid);
