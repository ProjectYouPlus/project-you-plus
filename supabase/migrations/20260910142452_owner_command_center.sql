...
for insert to authenticated with check ((select private.is_app_owner()));
create policy "owners can update admin users" on public.admin_users
for update to authenticated using ((select private.is_app_owner())) with check ((select private.is_app_owner()));
create policy "owners can delete admin users" on public.admin_users
for delete to authenticated using ((select private.is_app_owner()));
...
create policy "admins can view user directory" on public.user_directory
for select to authenticated using ((select private.is_app_admin()));
...
create policy "admins can view user metadata" on public.user_admin_metadata
for select to authenticated using ((select private.is_app_admin()));
create policy "admins can insert user metadata" on public.user_admin_metadata
for insert to authenticated with check ((select private.is_app_admin()));
create policy "admins can update user metadata" on public.user_admin_metadata
for update to authenticated using ((select private.is_app_admin())) with check ((select private.is_app_admin()));
create policy "owners can delete user metadata" on public.user_admin_metadata
for delete to authenticated using ((select private.is_app_owner()));
...
create policy "admins can view login events" on public.login_events
for select to authenticated using ((select private.is_app_admin()));
...
for insert to authenticated with check ((select auth.uid()) = user_id);
create policy "admins can view activity" on public.activity_events
for select to authenticated using ((select private.is_app_admin()));
...
create policy "authenticated can view app modules" on public.app_modules
for select to authenticated using (true);
create policy "admins can insert app modules" on public.app_modules
for insert to authenticated with check ((select private.is_app_admin()));
create policy "admins can update app modules" on public.app_modules
for update to authenticated using ((select private.is_app_admin())) with check ((select private.is_app_admin()));
create policy "owners can delete app modules" on public.app_modules
for delete to authenticated using ((select private.is_app_owner()));
...
for select to anon using (public_readable = true);
create policy "authenticated can read public settings" on public.app_settings
for select to authenticated using (public_readable = true or (select private.is_app_admin()));
create policy "admins can insert settings" on public.app_settings
for insert to authenticated with check ((select private.is_app_admin()));
create policy "admins can update settings" on public.app_settings
for update to authenticated using ((select private.is_app_admin())) with check ((select private.is_app_admin()));
create policy "owners can delete settings" on public.app_settings
for delete to authenticated using ((select private.is_app_owner()));
...
begin
  ...
  on conflict (user_id) do update set
    ...
    full_name = coalesce(excluded.full_name, public.user_directory.full_name),
    provider = excluded.provider,
    last_sign_in_at = excluded.last_sign_in_at,
    email_confirmed_at = excluded.email_confirmed_at,
    updated_at = now();

  insert into public.user_admin_metadata(user_id)
  values(new.id)
...
after insert or update of email, last_sign_in_at, email_confirmed_at, raw_user_meta_data, raw_app_meta_data on auth.users
for each row execute function private.sync_user_directory();

insert into public.user_directory(user_id,email,full_name,provider,created_at,last_sign_in_at,email_confirmed_at,updated_at)
select u.id,u.email,coalesce(u.raw_user_meta_data->>'full_name',u.raw_user_meta_data->>'name'),coalesce(u.raw_app_meta_data->>'provider','email'),u.created_at,u.last_sign_in_at,u.email_confirmed_at,now()
from auth.users u
on conflict (user_id) do update set
  email=excluded.email,
  full_name=coalesce(excluded.full_name, public.user_directory.full_name),
...
begin
  ...
  on conflict (user_id) do update set
    last_seen_at = greatest(coalesce(public.user_admin_metadata.last_seen_at, '-infinity'::timestamptz), excluded.last_seen_at),
    last_path = coalesce(excluded.last_path, public.user_admin_metadata.last_path),
    updated_at = now();
  return new;
end;
$$;
...
create trigger login_events_touch_last_seen after insert on public.login_events
for each row execute function private.touch_user_last_seen();
drop trigger if exists activity_events_touch_last_seen on public.activity_events;
create trigger activity_events_touch_last_seen after insert on public.activity_events
for each row execute function private.touch_user_last_seen();
...
create trigger audit_app_modules after insert or update or delete on public.app_modules
...
create trigger audit_user_admin_metadata after insert or update or delete on public.user_admin_metadata
for each row execute function private.audit_owner_change();

insert into public.app_modules(module_key,label,description,enabled,rollout_percent,locked)
values
...
insert into public.app_settings(setting_key,label,description,category,value,public_readable)
values
