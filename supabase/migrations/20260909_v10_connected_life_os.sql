-- Project You+ V10: reminders, accountability, challenges, and secure integration plumbing.
...
as $$
  select exists(select 1 from public.challenge_members cm where cm.challenge_id = p_challenge_id and cm.user_id = auth.uid());
$$;
...
-- Service-only token storage. RLS is intentionally enabled without client policies.
