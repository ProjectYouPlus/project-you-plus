...
export async function createTask(formData: FormData) {
  ...
  if (isDemoMode) {
    ...
    demoTasks.unshift({ id: `demo-${Date.now()}`, goalId, title, tier, dueAt, completedAt: null, meta: goalId ? "Goal-linked" : "" });
  ...
  const { error } = await supabase.from("tasks").insert({ user_id: user.id, title, tier, due_at: dueAt, goal_id: goalId });
...
export async function toggleTaskComplete(taskId: string, completed: boolean) {
  ...
  const supabase = await createClient();
  await supabase.from("tasks").update({ completed_at: completed ? new Date().toISOString() : null }).eq("id", taskId);
  revalidatePath("/tasks");
  revalidatePath("/dashboard");
...
export async function updateTask(taskId: string, updates: { title?: string; tier?: Tier; dueAt?: string | null; goalId?: string | null }) {
  if (isDemoMode) return;
  const supabase = await createClient();
