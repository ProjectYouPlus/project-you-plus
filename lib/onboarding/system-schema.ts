...
export type SystemGoalCheckpoint = { linkedGoalClientId:string; frequency:"weekly"|"monthly"; label:string };
export type SystemReview = { day:number; time:string; reminderIntent:boolean; rationale:string; goalCheckpointFrequency:"weekly"|"monthly"; goalCheckpoints?:SystemGoalCheckpoint[]; healthCheckIn:boolean; financeCheckIn:boolean; questions:string[] };
...
export function validateSystemProposal(value:unknown,answers?:OnboardingAnswers):ValidationResult{
  ...
  for(const checkpoint of proposal.weeklyReview.goalCheckpoints??[]){if(!goalIds.has(checkpoint.linkedGoalClientId)||!checkpoint.label.trim()||!(["weekly","monthly"] as const).includes(checkpoint.frequency))issues.push("review_goal_checkpoint");}
