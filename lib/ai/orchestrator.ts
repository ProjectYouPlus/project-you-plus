...
function fallbackFinding(agent:Specialist,context:UserContext,message:string):SpecialistFinding{
 ...
 if(agent==="finance"){
  ...
  const budgetRoom=finance.budgetRemaining,upcoming=finance.upcomingBills.total;
  const observation=amount!=null&&balance!=null?`Your Finance Score is ${finance.score}. You have ${money(balance)} in connected liquid cash${budgetRoom!=null?`, ${money(budgetRoom)} remaining in this month’s budget`:""}${upcoming!=null?`, and ${money(upcoming)} in tracked upcoming bills`:""}. The ${money(amount)} purchase would use ${Math.round(amount/Math.max(1,balance)*100)}% of liquid cash${budgetRoom!=null?` and ${Math.round(amount/Math.max(1,budgetRoom)*100)}% of remaining budget`:""}.`:`Your Finance Score is ${finance.score}. Project You+ has ${finance.accountCount} connected financial account${finance.accountCount===1?"":"s"}${budgetRoom!=null?` and ${money(budgetRoom)} of monthly budget remaining`:""}.`;
  const fits=amount!=null&&balance!=null&&amount<=balance&&(budgetRoom==null||amount<=budgetRoom)&&(upcoming==null||amount+upcoming<=balance);
  const suggestedAction=amount==null?"Compare the decision with liquid cash, remaining budget, upcoming bills, and goal pace before committing.":fits?"The purchase fits the currently connected cash and budget signals. Confirm bill tracking is complete before committing.":"Delay or reduce the purchase because it exceeds at least one current cash, budget, or bill-coverage signal.";
  const unavailable=[...(balance==null?["No checking, savings, cash, or depository balance is available."]:[]),...(budgetRoom==null?["Monthly budget room is unavailable."]:[]),...(finance.upcomingBills.count===0?["No upcoming bills are tracked."]:[])];
  return{agent,observations:[{id:hash(`finance:${amount}:${balance}:${budgetRoom}:${upcoming}`).slice(0,16),agent,domain:"finance",observation,evidence,confidence:evidence.length&&balance!=null?"medium":"low",suggestedAction,expectedImpact:"Protects cash flow by checking the decision against connected cash, spending pace, obligations, and the active savings goal.",relatedEntityIds:evidence.flatMap(item=>item.sourceId?[item.sourceId]:[])}],unavailable,proposedChanges:[]};
 }
