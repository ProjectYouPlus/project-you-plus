...
export function BetaForm() {
  ...
  return <form className="beta-form" onSubmit={submit} onFocusCapture={start} aria-label="Join the private beta">
    ...
    <fieldset><legend>Would you pay $19.99/month for YOU+ if it consistently helped you make meaningful progress? <span>Optional</span></legend><div className="pricing-options">{["Yes", "Maybe", "No"].map(value => <label key={value}><input type="radio" name="willingness_to_pay" value={value.toLowerCase()} /><span>{value}</span></label>)}</div><p className="research-note">A research question. No payment or commitment.</p></fieldset>
