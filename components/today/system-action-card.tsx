...
import { completeSystemTask, explainBlockedSystemTask, rescheduleSystemTask, skipSystemTask, useSystemMinimumVersion } from "@/lib/actions/today-system";
...
export function SystemActionCard({task,goalTitle,index,hero=false}:{task:Task;goalTitle?:string;index?:number;hero?:boolean}){
  ...
  function run(work:()=>Promise<{ok:boolean;error?:string}>){setError(null);startTransition(async()=>{const result=await work();if(!result.ok){setError(result.error??"Could not update this action.");return;}setMode("none");router.refresh();});}
  const body=<>
