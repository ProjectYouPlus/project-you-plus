...
test("a failed request preserves form values and can be retried", async ({ page }) => {
  ...
  await page.goto("/#beta");await page.locator("#beta-name").fill("Website QA");await page.locator("#beta-email").fill("website-qa@example.invalid");await page.locator("#beta-goal").fill("Consistency");await page.locator(".form-submit").click();
